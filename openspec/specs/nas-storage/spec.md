# nas-storage Specification

## Purpose

Defines how the `jg-jiahd` cluster provisions all persistent (NFS) storage from a single NAS appliance, and the data-continuity and monitoring guarantees that hold after migrating off the old NAS. Created by archiving change `use-new-nas-only`.
